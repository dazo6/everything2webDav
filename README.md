...
通过本项目，你可以使用任何支持 WebDAV 的客户端（如 CX文件管理器、FE文件管理器、流舟文件 等）直接浏览、搜索、播放和下载 Everything 索引的文件。
...
*   **`base.path.map`**: 这是一个非常重要的配置。有时 Everything 返回的路径（本地绝对路径）可能需要映射才能被正确访问，或者你希望隐藏真实的盘符。
    ...
    *   如果遇到 `Cannot convert value of type 'java.lang.String' to required type 'java.util.Map'` 错误，请确保使用本项目提供的最新代码（已包含自定义解析逻辑）。
...
这将生成 `target/everything2webDav-1.0-SNAPSHOT.jar`。
...
*注意：必须添加 `-Dfile.encoding=UTF-8` 参数，否则在 Windows CMD 中可能会出现日志乱码。*
